...
from landcover_lca.models import (
...
class DatasetLoadingTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.transition_matrix = pd.read_csv(
            os.path.join(self.data_dir, "transition.csv"), index_col=0
        )
        self.land_uses = pd.read_csv(
            os.path.join(self.data_dir, "land_uses.csv"), index_col=0
        )
